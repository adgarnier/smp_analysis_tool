...
tool_version = 'SMP Analysis Tool v7.1.2'

# logging.info(f'Running {tool_version}')
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
       ...
       # Sign section inside frame_col3_top
        self.sign_button = tk.Button(frame_col3_top, text="Sign", command=self.signing, width=button_width)
        self.sign_button.grid(row=0, column=0, padx=10, pady=10, columnspan=2)
    ...
    def variables(self):
        ...
        try:
            ...
            logging.info("task is not set or invalid")

        try:
            self.tool_version = tool_version
        except ValueError:
            logging.info("tool is not set or invalid")

        # Create config_data using instance attributes
        variables = ['plate_amt', 'file_amt', 'directory', 'x_cord', 'y_cord', \
                    'export_setting', 'export_setting2', 'task', 'tool_version']
        config_data = {}
    ...
    def signing(self):
        ...
        if type(self.plate_amt) == int:
            logging.info('Running sign...')
            self._run_script("tk_10_signstatement.pyw")
        elif type(self.plate_amt) != int:
            logging.info('print plate_amt not set')
            messagebox.showinfo("Info", "Please input a valid number of plates.")        
        else:
            logging.info('Error in sign')
    ...
    def rawdata(self):
        ...
        self.variables()
        if type(self.plate_amt) == int:
            logging.info('Running raw data...')
            self._run_script("tk_09_rawdata.pyw")
        elif type(self.plate_amt) != int:
            logging.info('print plate_amt not set')
            messagebox.showinfo("Info", "Please input a valid number of plates.")        
        else:
            logging.info('Error in raw data')
    ...
    def save(self):
        ...
        if type(self.plate_amt) == int:
            ...
            self._run_script("tk_11_save.pyw")
...
Open: Opens the plates.\n\
* Start by opening the final file in the folder.\n\
...
Sign: Signs the plates and submits for approval. \n\n\
