...
class Application(tk.Frame):
    ...
    def variables(self):
        ...
        variables = ['plate_amt', 'file_amt', 'directory', 'setting_num', 'x_cord', 'y_cord', 'export_setting', 'export_setting2']
        config_data = {}
            ...
            if selected_indices == (0, 1, 2) or selected_indices == (0, 2):
                ...
                try: 
                    if 0 in selected_indices or 7 in selected_indices2:
                        if not self.directory or self.directory == None:
                            ...
                            print(f'doing excel from {selected_items} or {selected_items2}')
                ...
                try:
                    if 1 in selected_indices or 6 in selected_indices2:
                        print(f'doing lims from {selected_items} or {selected_items2}')
                        self._run_script("tk_03_export.py")
                except ValueError:
                    print('oops')
                try:
                    if 2 in selected_indices or 8 in selected_indices2:
                        if not self.directory or self.directory == None:
                            ...
                            print(f'doing pnps from {selected_items} or {selected_items2}')
                            self._run_script("tk_03_export.py")                            
                        else:
                            messagebox.showinfo("Info", "Please select save directory.")
                except ValueError:
                ...
                for index in selected_indices2:
                    if index in actions and not self.script_aborted:
                        ...
                        if callable(action_method):
                            print(f'Doing {actions[index]}...')
                            action_method()
                        time.sleep(1)
                        while go == 1:
                            try:
                                # print('looking for abort status')
                                if self.find_window_by_title('Abort Status'):
                                    print("Found 'Abort Status'")
                                    return
                                # print('did not find abort')
                            except Exception as e:
                                print(f'Error checking abort status: {e}')
                            try:
                                # print('looking for script stat')
                                if self.find_window_by_title('Script Status'):
                                    print("Found 'Scrip Status'")
                                    go = 0
...
root = tk.Tk()
root.title("SMP Analysis Tool v2.2")
